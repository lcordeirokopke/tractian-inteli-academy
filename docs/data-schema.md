...
## Mapeamento parquet → JSON da API (achatamento vs. aninhamento)

As tabelas parquet guardam os campos técnicos de `assets` e `models` **soltos** (uma coluna por
campo), formato mais simples de gerar/ler com pandas/duckdb. O contrato OpenAPI
(`docs/api-contract.openapi.yaml`) organiza esses mesmos campos em **objetos aninhados** na
resposta JSON, o que é mais legível para quem consome a API. A implementação da API (fase
posterior) precisa aplicar essa transformação explicitamente ao montar a resposta a partir da
linha do parquet — atenção especial ao campo `bearing_pn`, que **muda de nome** (não só de
posição).

### `assets.parquet` → `Asset`
| Coluna (parquet)                          | Campo (JSON)                          |
| :----------------------------------------- | :------------------------------------- |
| `plant`                                    | `hierarchy.plant`                      |
| `line`                                     | `hierarchy.line`                       |
| `parent_asset_id`                          | `hierarchy.parent_asset_id`            |
| `machine_type`                             | `config.machine_type`                  |
| `rotation_rpm`                             | `config.rotation_rpm`                  |
| `bearing_pn`                               | `config.bearing_specs.part_number` - renomeado |
| `bpfo_hz`                                  | `config.bearing_specs.bpfo_hz`         |
| `bpfi_hz`                                  | `config.bearing_specs.bpfi_hz`         |
| `bsf_hz`                                   | `config.bearing_specs.bsf_hz`          |
| `ftf_hz`                                   | `config.bearing_specs.ftf_hz`          |
| `line_frequency_hz`                        | `config.line_frequency_hz`             |

### `models.parquet` → `Model`
| Coluna (parquet)   | Campo (JSON)                |
| :------------------ | :--------------------------- |
| `min_completeness`  | `requirements.min_completeness` |
| `min_snr_db`        | `requirements.min_snr_db`       |
| `min_rotation_rpm`  | `requirements.min_rotation_rpm` |

Demais colunas de ambas as tabelas (ex.: `id`, `name`, `company_id`, `criticality`, `version`,
`coverage`, `processing_state`, `last_run_at`) mapeiam 1:1 para o campo de mesmo nome no JSON,
sem aninhamento.
